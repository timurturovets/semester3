...
public static class Task1
{
    public static void Run(string[] args)
    {
        ...
        var f = x * y.Pow(3) - z.Pow(2) + y.Pow(5) - z.Pow(3);
        var g1 = -x.Pow(3) + y;
        var g2 = x.Pow(2) * y - z;

        var generators = new List<Polynomial> { g1, g2 };
        var groebnerBasis = BuildGroebnerBasis(generators);
        var remainder = f.ReduceBy(groebnerBasis);

        Console.WriteLine("Проверка принадлежности полинома f идеалу I = < -x^3 + y, x^2y - z >");
